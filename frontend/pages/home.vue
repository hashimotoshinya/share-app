<template>
  <div class="flex bg-gray-900 min-h-screen text-white">
    ...
    <div class="flex-1 flex flex-col h-screen pr-10">
      ...
      <div class="pt-8 sticky top-0 bg-gray-900 z-10 border-l border-white flex items-center">
...
const deletePost = async (postId) => {
  try {
    ...
    if (status === 403) {
      notification.value = { message: '投稿者以外は削除できません。', type: 'error' }
    } else if (status === 401) {
      notification.value = { message: '認証に失敗しました。再ログインしてください。', type: 'error' }
    } else {
      notification.value = { message: serverMsg || '削除に失敗しました。', type: 'error' }
    }
